...
const FilterMenu = ({ onFilterChange }) => {
    ...
    const filteredLocations = locations.filter(location =>
        ...
        location.toLowerCase().includes(filters.location ? filters.location.toLowerCase() : '')
    );
    ...
    return (
        <div className="sticky left-0 top-0 h-screen w-[22rem] p-4 shadow-lg z-20 overflow-y-auto" style={{ backgroundColor: '#050505', borderRight: '1px solid #AD6F3D' }}>
            ...
            <div className="mb-4">
                <label htmlFor="location" className="block text-sm font-medium" style={{ color: '#F1ECE9' }}>Localisations :</label>
                    ...
                    {filters.location && (
                        <ul className="absolute top-full left-0 right-0 z-10 bg-white border rounded-md shadow-md">
                            {filteredLocations.map((location, index) => (
                                <li key={index} className="cursor-pointer p-2 hover:bg-gray-100" onClick={() => handleLocationClick(location)}>
                                    {location}
                                </li>
                            ))}
                        </ul>
                ...
                <div className="flex flex-wrap mt-2">
                    {filters.locations.map((location, index) => (
                        <div key={index} className=" text-white p-2 rounded-md m-1 flex items-center" style={{ backgroundColor: '#AD6F3D' }}>
                            <span className="mr-2">{location}</span>
                            <button type="button" onClick={() => handleLocationRemove(location)} className="text-white">
                                &#10005;
                            </button>
                        </div>
                    ))}
            ...
            <div className="mb-4">
                <label htmlFor="participants" className="block text-sm font-medium" style={{ color: '#F1ECE9' }}>Places disponibles :</label>
                <input
                    id="participants"
                    name="participants"
            ...
            <div className="mb-4">
                <label htmlFor="dateRange" className="block text-sm font-medium my-2" style={{ color: '#F1ECE9' }}>À partir de</label>
                <DateRangePicker
            ...
            <div className="mb-4">
                <label htmlFor="tag" className="block text-sm font-medium" style={{ color: '#F1ECE9' }}>Tag :</label>
                <div className="relative">
                    <input
                        id="tag"
                        name="tag"
